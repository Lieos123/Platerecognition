...
{
    int radius = 1;  // LBP 半径
    int neighbors = 8;  // LBP 邻域数

    //Mat gray;
    //cvtColor(img, gray, COLOR_BGR2GRAY);

    lbpImg = Mat::zeros(img.rows, img.cols, CV_8UC1);

    //elbp(gray, lbpImg, radius, neighbors);
    elbp(img, lbpImg, radius, neighbors);
...
void extractLBPs(const vector<Mat>& images, vector<Mat>& lbpImages)
{
    ...
    for (size_t i = 0; i < images.size(); i++) {
        ...
        // 创建空的输出图像
        Mat lbpImg = Mat::zeros(img.rows, img.cols, CV_8UC1);

        // 提取 LBP 特征
        elbp(gray, lbpImg, radius, neighbors);

        // 将提取的 LBP 特征加入到结果向量中
        lbpImages.push_back(lbpImg);
    }
}